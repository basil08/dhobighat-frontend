...
import { ChevronDownIcon, ChevronRightIcon, ArchiveIcon, RotateCcwIcon } from 'lucide-react';
...
export default function ArchivePage() {
  ...
  const loadArchivedItems = async () => {
    try {
      setLoading(true);
      const data = await clothingApi.getArchivedItems();
      setItemsByType(data);
      // Expand all types by default
      setExpandedTypes(new Set(Object.keys(data)));
    } catch (err) {
      setError('Failed to load archived clothing items');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleUnarchive = async (itemId: string) => {
    try {
      setUnarchiving(prev => new Set(prev).add(itemId));
      await clothingApi.unarchiveItem(itemId);
    ...
    <ProtectedRoute>
      ...
      {Object.keys(itemsByType).length === 0 ? (
        <div className="text-center py-8 sm:py-12">
          ...
          <p className="text-gray-500 text-base sm:text-lg">No archived clothing items found.</p>
          <Link
            href="/"
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2 ml-2">
                            <button
                              ...
                              disabled={unarchiving.has(item.id)}
                              className="flex items-center space-x-1 bg-blue-600 text-white px-3 py-1 rounded-full text-xs sm:text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
                              <RotateCcwIcon className="h-3 w-3 sm:h-4 sm:w-4" />
                              <span className="hidden sm:inline">
                                {unarchiving.has(item.id) ? 'Unarchiving...' : 'Unarchive'}
                              </span>
                            </button>
                          </div>
                        </div>
